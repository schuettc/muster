...
**Push delivery (optional, additive):** `claude mcp add muster-channel -s user -- muster channel` registers the channel carrier beside `muster mcp` — new mail then lands in the session as a push instead of waiting for the Stop-hook drain. The hooks above stay exactly as configured; they are the fallback when no channel is attached.
