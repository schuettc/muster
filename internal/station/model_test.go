...
func TestFetchThreadsRequestsActiveTasksAndDecodesTruncation(t *testing.T) {
	caller := fakeCaller{fn: func(op string, args map[string]any) (json.RawMessage, error) {
		...
		return json.RawMessage(`{"threads":[{"id":7}],"active_tasks_truncated":true,"retained_active_task_ids":[7]}`), nil
	}}
	msg, ok := fetchThreadsCmd(caller)().(threadsMsg)
	if !ok {
		t.Fatalf("fetchThreadsCmd returned %T", fetchThreadsCmd(caller)())
	}
	if msg.err != nil || len(msg.threads) != 1 || msg.threads[0].ID != 7 || !msg.threads[0].RetainedActiveTask || !msg.activeTasksTruncated {
		t.Fatalf("threadsMsg = %+v", msg)
	}
}

func TestActiveTaskTruncationAppearsWithoutOverwritingActionStatus(t *testing.T) {
	m := NewModel(fakeCaller{}, Options{})
	m.status = "task #7 update failed: conflict"
	next, _ := m.Update(threadsMsg{activeTasksTruncated: true})
	m = mustModel(t, next)
	if m.status != "task #7 update failed: conflict" {
		t.Fatalf("action status was overwritten: %q", m.status)
	}
	view := m.renderStatus()
	if !strings.Contains(view, "conflict") || !strings.Contains(view, "active task list truncated at 500") {
		t.Fatalf("status line does not preserve both messages: %q", view)
	}
}
