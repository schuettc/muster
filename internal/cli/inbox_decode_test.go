...
// TestDecodeInboxResponseLegacyBareArray: Finding 2. A 0.13.0+ client talking
// to a still-running pre-0.13.0 daemon (or a device forwarding to a
// not-yet-redeployed hosted lambda) gets the OLD bare-array get_inbox shape,
// not {threads, marked_read}. The decode must fall back to that shape and
// report marked_read=true — exactly what the old daemon actually did on
// every read — rather than hard-failing.
