...
**Local-first shadows a foreign bare alias.** A foreign agent whose alias strips to one of ours cannot be reached by that bare string from this machine. Under decision 5 this only arises for rows registered before auto-adoption; collision-aware rendering makes the situation visible, and the full alias still resolves exactly.
